...
// ID is a kstack peer identity.
//
// kstack defines three types of peer ID:
// 1) An anonymous temporary ID wraps UUIDv4 with Identity hashing;
// 2) An inlined ID wraps Ed25519 pubkey with Identity hashing;
// 3) An digested ID wraps other pubkeys with SHA2-256 hashing.
type ID string
...
// the public key.
func (id ID) ExtractPublicKey() (kc.PubKey, error) {
	if id.IsAnonymous() {
	...
	pk, err := kc.UnmarshalPublicKey(decoded.Digest)
	if err != nil {
		return nil, ErrNoPublicKey
	}
	return pk, nil
...
func IDFromBytes(b []byte) (ID, error) {
	if _, err := ID(b).validateAnonymous(); err != nil {
		return ID(""), err
	}
...
func Decode(s string) (ID, error) {
	...
	return "", fmt.Errorf("failed to parse peer ID: bad format")
...
func IDFromPublicKey(pk kc.PubKey) (ID, error) {
	b, err := kc.MarshalPublicKey(pk)
	if err != nil {
		return "", err
	}
...
func IDFromPrivateKey(sk kc.PrivKey) (ID, error) {
